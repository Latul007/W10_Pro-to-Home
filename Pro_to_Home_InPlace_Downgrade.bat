@echo off
:: Pro -> Home in-place edition switcher (Windows 10)
:: Version: 2025-08-17
:: Requires: mounted Windows 10 Home ISO
...
echo.
echo ===============================================================
echo   Windows 10 Pro -> Home (in-place) Edition Switcher
...
set "TS=%TS: =0%"
set "BKP=%TEMP%\win10_edition_backup_%TS%.reg"
...
reg add "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion" /v ProductName /t REG_SZ /d "Windows 10 Home" /f >nul
...
if not defined SETUP (
    echo.
    echo   Could not find Windows 10 setup on mounted media.
    echo   -> Mount a Windows 10 Home ISO, then run this script again.
...
echo [4/4] Launching Windows 10 Setup from: %SETUP%
echo     When prompted, choose "Keep personal files and apps".
start "" "%SETUP%"

echo.
echo === Next steps ===============================================
echo 1) Complete the in-place upgrade to Windows 10 Home.
echo 2) After reboot, activate with a valid Windows 10 Home license.
